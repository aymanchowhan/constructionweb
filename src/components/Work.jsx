...
function Work() {
  ...
  return (
    < div id='services' className='min-h-screen bg-blue-100 m-auto container pt-[90px] pb-[70px]' >
      <div className='text-center text-[36px]'>
        Featured Work
      </div>
      <div className='text-center px-2 text-[14px] lg:w-1/2 mx-auto opacity-50 pt-10'>
          ...
          data.map((item, index) =>
            <div key={index} onMouseOver={() => setActiveIndex(index)} onMouseOut={() => setActiveIndex(null)}
              ...
              <div className={` ${activeIndex == index && 'absolute top-0 right-0 left-0 bottom-0 bg-[#f5af0070] '} `}>
                <div className='text-white text-center text-[14px] py-2'>{item.name}</div>
              </div>
            </div>
          )
        }
